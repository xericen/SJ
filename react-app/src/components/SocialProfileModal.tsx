...
export function SocialProfileModal({person,location,isFriend,friendRequestPending=false,blockMode,openReportInitially=false,onClose,onToggleFriend,onDirectChat,onReport}:{person:SocialProfilePerson;location:string;isFriend:boolean;friendRequestPending?:boolean;blockMode:SocialBlockMode;openReportInitially?:boolean;onClose:()=>void;onToggleFriend:()=>void;onDirectChat:()=>void;onReport:(reason:SocialReportReason,mode:SocialBlockMode,detail:string)=>void}){
  ...
  return <div className="social-profile-overlay" role="dialog" aria-modal="true" aria-labelledby="social-profile-title" onClick={onClose}>
    <section className="social-profile" onClick={event=>event.stopPropagation()}>
      ...
      <header className="social-profile-hero">
        <div className="social-profile-identity"><div className="social-profile-avatar"><CharacterPreview parts={person.appearance} small/><i/></div><div className="social-profile-copy"><small>{person.kind==='npc'?'세종에서 만난 NPC':'세종을 함께 탐험하는 친구'}</small><h2 id="social-profile-title">친구 프로필 <Sparkles size={18}/></h2><div className="social-profile-name">{person.nickname}<span>{isFriend?'내 친구':'새로운 이웃'}</span></div><div className="social-profile-basic-info"><span><small>현재 위치</small>{location}</span><span><small>프로필</small>{person.kind==='npc'?'로컬 가이드 NPC':'실시간 사용자'}</span>{person.mbti&&<span><small>MBTI</small>{person.mbti}</span>}</div><p>{person.status}</p></div></div>
        <div className="social-profile-actions"><button type="button" className={isFriend?'is-friend':''} disabled={friendRequestPending} onClick={onToggleFriend}>{isFriend?<UserMinus/>:<UserPlus/>}{isFriend?'친구 삭제':friendRequestPending?'상대 수락 대기 중':'친구 요청 보내기'}</button><button type="button" disabled={blocked} onClick={onDirectChat}>{blocked?<MessageCircleOff/>:<MessageCircle/>}{blocked?'대화 차단됨':'1:1 대화'}</button></div>
      </header>
