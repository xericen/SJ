...
export function PersonalFarmProgressExperience({mapId,userKey,authenticated,onNotice}:{mapId:MapId;userKey:string;authenticated:boolean;onNotice?:(message:string)=>void}){
  ...
  return <div className={`personal-farm-mission-ui map-${mapId}`} aria-live="polite">
    {mapId==='personal-farm'&&progress&&farmSlot&&<section className="personal-farm-nearby-slot-picker">
      <header><b>{farmSlot.slot}번 꽃 심기 자리</b><span>{farmSlot.flowerId?`${plantName(farmSlot.flowerId)} 교체`:'반짝이는 빈 자리'}</span></header>
      <footer><select value={selectedFlower} onChange={event=>setSelectedFlower(event.target.value as GardenFlowerId|'')}><option value="">심을 꽃 선택</option>{availableToPlant.map(id=><option value={id} key={id}>{plantName(id)}</option>)}</select><button type="button" disabled={!selectedFlower||!!pending} onClick={()=>selectedFlower&&void run(`slot:${farmSlot.slot}`,()=>plantGardenFlowerInSlot(selectedFlower,farmSlot.slot),`${farmSlot.slot}번 자리에 ${plantName(selectedFlower)}을(를) 심었어요.`).then(()=>setSelectedFlower(''))}>{pending===`slot:${farmSlot.slot}`?'심는 중…':farmSlot.flowerId?'꽃 교체하기':'꽃 심기'}</button></footer>
    </section>}
