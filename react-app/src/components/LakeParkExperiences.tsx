...
type FestivalTimelineItem={time:string;title:string;detail:string};
type FestivalZoneItem={name:string;detail:string};
type FestivalVisitInfo={dayNight:string;programs:string[];recommendation:string;price:string;recommendedTime:string;congestion:string;nearby:string;transport:string;parking:string;supplies:string;timeline:FestivalTimelineItem[];zones:FestivalZoneItem[];course:string[];mapAddress:string;arrivalTip:string;returnTip:string};
...
const festivalVisitInfo:Record<string,FestivalVisitInfo>={
  'peach-festival':{dayNight:'주간·저녁',programs:['복숭아 판촉전','가족 체험','지역 공연'],recommendation:'가족·친구',price:'입장 무료',recommendedTime:'11:00~17:30',congestion:'토요일 13~16시 판매 부스 혼잡',nearby:'도도리파크·세종전통시장',transport:'조치원역에서 행사장 방면 셔틀·시내버스를 이용하고, 하차 후 안내 표지를 따라 도보 이동하세요.',parking:'세종시민운동장 및 행사 임시주차장을 이용하되 오후에는 대중교통을 권장해요.',supplies:'모자, 장바구니, 물, 보냉가방',timeline:[{time:'10:30',title:'행사장 도착',detail:'종합안내소에서 당일 공연과 체험 접수 시간을 먼저 확인해요.'},{time:'11:00',title:'복숭아 판매장',detail:'농가별 복숭아를 비교하고 시식·판매 부스를 둘러봐요.'},{time:'13:00',title:'가족 체험',detail:'복숭아를 주제로 한 만들기와 참여 프로그램을 체험해요.'},{time:'15:00',title:'메인 공연',detail:'보조경기장 무대의 지역 공연과 시민 프로그램을 관람해요.'},{time:'16:30',title:'도도리파크 연계',detail:'행사장을 나와 인근 농촌 테마 공간까지 여유롭게 둘러봐요.'}],zones:[{name:'종합안내소',detail:'일정표·분실물·의무실 안내'},{name:'복숭아 판매장',detail:'농가 판매와 시식 부스'},{name:'체험마당',detail:'가족 참여 프로그램 접수'},{name:'메인무대',detail:'공연과 시민 이벤트'},{name:'먹거리·휴게존',detail:'지역 먹거리와 그늘 쉼터'}],course:['조치원역','세종시민운동장','도도리파크'],mapAddress:'세종시민운동장 보조경기장',arrivalTip:'오전 11시 이전 도착하면 판매·체험 대기가 비교적 짧아요.',returnTip:'공연 종료 직후보다 15~20분 일찍 이동하면 셔틀 혼잡을 줄일 수 있어요.'},
  'hangeul-festival':{dayNight:'낮·야간',programs:['한글 주제 공연','미디어아트 전시','시민 참여 체험'],recommendation:'가족·연인·친구',price:'무료',recommendedTime:'16:30~20:30',congestion:'한글날 18시 이후 혼잡',nearby:'국립세종수목원',transport:'BRT 정부세종청사·세종호수공원 정류장에서 하차한 뒤 행사 안내 동선을 따라 이동하세요.',parking:'호수공원 제1·2주차장, 만차 시 중앙공원 임시주차장을 이용하세요.',supplies:'편한 신발, 얇은 겉옷, 보조배터리',timeline:[{time:'16:30',title:'한글 체험',detail:'체험마을에서 참여 프로그램을 먼저 예약해요.'},{time:'17:30',title:'한글광장 전시',detail:'한글 조형물과 시민 전시를 천천히 관람해요.'},{time:'18:30',title:'주제 공연',detail:'메인무대의 대표 공연을 관람해요.'},{time:'19:30',title:'미디어아트',detail:'호수 주변 야간 콘텐츠와 빛 전시를 즐겨요.'},{time:'20:20',title:'분산 귀가',detail:'중앙공원 방향 출구로 이동해 혼잡을 피해요.'}],zones:[{name:'한글광장',detail:'주제 전시와 포토존'},{name:'공연무대',detail:'대표 공연과 시민 무대'},{name:'체험마을',detail:'한글 놀이·만들기 접수'},{name:'푸드존',detail:'지역 먹거리와 휴게 공간'}],course:['국립세종수목원','세종호수공원','한글축제 야간공연'],mapAddress:'세종호수공원',arrivalTip:'오후 4시 30분 전 도착해 체험을 예약한 뒤 야간 공연으로 이동하세요.',returnTip:'메인 공연 종료 전 중앙공원 방향으로 이동하면 BRT 대기를 줄일 수 있어요.'},
  'nakhwa-festival':{dayNight:'야간',programs:['전통 낙화 시연','호수 야간 공연','문화 체험'],recommendation:'연인·친구·가족',price:'무료',recommendedTime:'18:00~21:00',congestion:'토요일 19시 이후 매우 혼잡',nearby:'국립세종수목원·이응다리',transport:'BRT 세종호수공원 정류장을 이용하고 통제 구간에서는 현장 안전요원의 안내를 따라주세요.',parking:'중앙공원 임시주차장 이용 후 행사장까지 도보 또는 셔틀로 이동하세요.',supplies:'돗자리, 얇은 겉옷, 물',timeline:[{time:'18:00',title:'관람구역 확인',detail:'안전통제선과 귀가 출구를 먼저 확인해요.'},{time:'18:30',title:'자리 잡기',detail:'지정 관람구역에서 시야와 안전거리를 확보해요.'},{time:'19:20',title:'식전 공연',detail:'메인무대의 전통 공연을 관람해요.'},{time:'20:00',title:'낙화 시연',detail:'안내 방송에 따라 이동 없이 낙화를 감상해요.'},{time:'20:40',title:'순차 퇴장',detail:'가까운 출구 대신 안내된 분산 동선으로 이동해요.'}],zones:[{name:'낙화 관람구역',detail:'지정 좌석·안전 관람선'},{name:'메인무대',detail:'식전 전통 공연'},{name:'문화 체험존',detail:'공예와 전통놀이'},{name:'안전통제구역',detail:'출입 제한·의무 지원'}],course:['국립세종수목원','이응다리','세종낙화축제'],mapAddress:'세종호수공원',arrivalTip:'해가 지기 전 도착해 관람구역과 비상 동선을 먼저 확인하세요.',returnTip:'시연 직후에는 혼잡하므로 안전요원의 구역별 퇴장 안내를 따라주세요.'},
};
const getFestivalVisitInfo=(festival:FestivalCard):FestivalVisitInfo=>festivalVisitInfo[festival.id]??{dayNight:festival.tags.some(tag=>tag.includes('야간'))?'야간':'주간',programs:[festival.category==='공연'?'대표 공연':'시민 참여 프로그램','지역 문화 전시','체험 부스'],recommendation:festival.tags.some(tag=>tag.includes('가족')||tag.includes('어린이'))?'가족':'친구·연인',price:'무료',recommendedTime:festival.category==='공연'?'16:30~20:30':'12:30~17:30',congestion:'주말 대표 프로그램 시작 1시간 전 혼잡',nearby:festival.venue.includes('호수공원')?'국립세종수목원':'세종전통시장',transport:'행사장 인근 BRT·시내버스 정류장에서 하차한 뒤 현장 안내 표지를 따라 이동하세요.',parking:'행사장 안내 주차장과 임시주차장을 이용하고 혼잡 시간에는 대중교통을 권장해요.',supplies:'편한 신발, 물, 보조배터리',timeline:[{time:'12:30',title:'행사장 도착',detail:'종합안내소에서 당일 운영 시간과 접수 프로그램을 확인해요.'},{time:'13:00',title:'체험 사전 접수',detail:'마감이 빠른 참여 프로그램을 먼저 예약해요.'},{time:'15:00',title:'핵심 프로그램',detail:'메인무대 또는 대표 전시를 중심으로 관람해요.'},{time:'16:30',title:'먹거리·휴식',detail:'휴게존에서 쉬며 남은 프로그램을 확인해요.'},{time:'17:30',title:'주변 관광·귀가',detail:'혼잡 전에 주변 명소 또는 대중교통 정류장으로 이동해요.'}],zones:[{name:'종합안내소',detail:'일정·접수·안전 안내'},{name:'메인무대',detail:'대표 공연과 행사'},{name:'체험존',detail:'시민 참여 프로그램'},{name:'휴게·먹거리존',detail:'식음과 휴식 공간'}],course:[festival.venue.split('·')[0],festival.title,'주변 관광지'],mapAddress:festival.venue.split('·')[0],arrivalTip:'대표 프로그램 시작 1시간 전에 도착하면 접수와 이동이 여유로워요.',returnTip:'종료 직후 혼잡을 피해 가까운 대중교통 정류장 위치를 미리 확인하세요.'};
...
const festivalCultureExperiences=[
  {id:'mask',emoji:'🎭',title:'전통 탈·놀이 체험',description:'세종의 전통 놀이와 탈 문화를 직접 체험해 보세요.',image:festivalImageUrl('/images/festivals/dano-2026.jpg',import.meta.env.BASE_URL)},
  {id:'craft',emoji:'🪭',title:'전통 공예 만들기',description:'축제의 색을 담은 나만의 전통 공예품을 만들어요.',image:festivalImageUrl('/images/festivals/spring-flower-2026.jpg',import.meta.env.BASE_URL)},
  {id:'hangeul',emoji:'📜',title:'한글 문화 체험',description:'세종대왕과 한글 이야기를 놀이와 전시로 만나보세요.',image:festivalImageUrl('/images/festivals/king-book-2026.jpg',import.meta.env.BASE_URL)},
];
...
export function LakeParkExperiences(){
  ...
  const [festivals,setFestivals]=useState<FestivalCard[]>(()=>fallbackFestivalContents.map(festival=>({...festival,image:festivalImageUrl(festival.image,import.meta.env.BASE_URL)})));
  ...
  const [festivalMapOpen,setFestivalMapOpen]=useState(false);
  ...
  const festivalPassportRef=useRef<HTMLElement|null>(null);
  ...
  const nextFestivalStamp=!festivalStamps.performance?'공연장 영상 보기':!festivalStamps.traditionalCulture?'전통문화 체험 완료':!festivalStamps.artExhibition?'한눈에 보기에서 관심 축제 저장':'모든 미션 완료';
  ...
  useEffect(()=>{
    ...
    fetch(`${API_BASE_URL}/festivals`,{signal:controller.signal}).then(response=>{
      ...
      const cards=payload.festivals.map((festival,index)=>{
        ...
        return {id:fallback?.id??festival.id,category:'축제' as const,emoji:fallback?.emoji??'🎪',image:festivalImageUrl(fallback?.image||festival.image||'/images/festivals/hangeul-2026.jpg',import.meta.env.BASE_URL),source:festival.source==='tour-api'?'한국관광공사 관광자료':festival.source==='sejong'?'세종특별자치시 공개자료':'세종 공식 2026 축제 데이터',title:festival.name,description:festival.description||fallback?.description||'세종에서 열리는 문화축제입니다.',schedule,venue:festival.venue||'세종특별자치시 일원',status:festival.status,tags:fallback?.tags??[festival.status,festival.organizer||'세종축제'].filter(Boolean),tone:fallback?.tone??['coral','violet','blue','green'][index%4]} satisfies FestivalCard;
      });
      setFestivals(cards);setFestivalDataSource('api');
    }).catch(error=>{if(error instanceof Error&&error.name!=='AbortError'){setFestivals(fallbackFestivalContents.map(festival=>({...festival,image:festivalImageUrl(festival.image,import.meta.env.BASE_URL)})));setFestivalDataSource('fallback')}});
  ...
  const selectFestivalVisitPlan=(id:string)=>{setFestivalVisitPlanId(id);setSelectedFestival(null);setFestivalMapOpen(false);const festival=festivals.find(item=>item.id===id);if(festival)recordExperienceAction({type:'festival-open',festivalId:id,festivalTitle:festival.title,categories:[...festival.tags,getFestivalVisitInfo(festival).dayNight,festival.category],location:festival.venue})};
  const inspectFestivalPlanSection=(section:'시간표'|'지도'|'교통')=>{setFestivalPlanSection(section);if(section!=='지도')setFestivalMapOpen(false);if(!festivalVisitPlan)return;const mapped=section==='시간표'?'timetable':section==='지도'?'map':'transport';recordExperienceAction({type:'festival-section',festivalId:festivalVisitPlan.id,festivalTitle:festivalVisitPlan.title,section:mapped})};
  ...
  useLayoutEffect(()=>{
    const passport=festivalPassportRef.current;
    const page=passport?.closest('.game-page') as HTMLElement|null;
    ...
    updateStack();
    const observer=typeof ResizeObserver==='undefined'?null:new ResizeObserver(updateStack);
    observer?.observe(passport);
    window.addEventListener('resize',updateStack);
    return()=>{observer?.disconnect();window.removeEventListener('resize',updateStack);page.style.removeProperty('--festival-passport-bottom')};
  },[isFestivalExperience,completedCount,allBoothsCompleted,nextFestivalStamp]);

  if(!supportsExperienceWeb)return null;
  return <>
    {isFestivalExperience&&<>
      <aside ref={festivalPassportRef} className={`festival-experience-passport ${allBoothsCompleted?'is-complete':''}`} role="status" aria-live="polite">
        <header><span>{allBoothsCompleted?'🏆':'🎟️'}</span><div><small>{allBoothsCompleted?'축제 미션 완료':'축제 스탬프 미션'}</small><b>{allBoothsCompleted?'3개 스탬프를 모두 받았어요':`${completedCount}/3 완료 · 다음: ${nextFestivalStamp}`}</b></div></header>
        <div>{([['공연장',festivalStamps.performance,'🎵'],['전통문화',festivalStamps.traditionalCulture,'🎭'],['한눈에 보기',festivalStamps.artExhibition,'🎨']] as const).map(([label,done,icon])=><span className={done?'done':''} key={label}><i>{done?'✓':icon}</i><small>{label}</small><em>{done?'완료':'대기'}</em></span>)}</div>
        <p>{allBoothsCompleted?'미션 완료! 축제부스의 세 가지 활동을 모두 체험했습니다.':`스탬프 ${3-completedCount}개가 남았어요. ${nextFestivalStamp}를 진행해 주세요.`}</p>
      </aside>
    ...
    {(active==='central-plaza'||active==='food-shop-zone'||active==='activity-zone'&&!isFestivalExperience)&&<div className="lake-experience-overlay festival-plaza-overlay" role="dialog" aria-modal="true" aria-labelledby="festival-title">
      <section className={`festival-plaza-panel ${isFestivalExperience?'is-festival-map':''} ${isFestivalExperience&&active==='activity-zone'?'is-festival-video':''}`}>
        ...
        {isFestivalExperience&&active==='central-plaza'&&festivalVisitPlan&&selectedFestivalVisitInfo&&<section className="festival-visit-dashboard">
          <nav className="festival-plan-picker" aria-label="축제 선택">{festivals.map((item,index)=><button type="button" className={item.id===festivalVisitPlan.id?'active':''} key={item.id} onClick={()=>selectFestivalVisitPlan(item.id)}><span>{item.emoji}</span><span><small>{String(index+1).padStart(2,'0')} FESTIVAL</small><b>{item.title.replace(/^2026\s*/, '')}</b></span></button>)}</nav>
          <div className="festival-visit-hero"><div><small>SEJONG FESTIVAL FIELD GUIDE</small><h3>{festivalVisitPlan.title}</h3><p>{festivalVisitPlan.description}</p><dl><div><dt>장소</dt><dd>{festivalVisitPlan.venue}</dd></div><div><dt>추천</dt><dd>{selectedFestivalVisitInfo.recommendedTime}</dd></div><div><dt>혼잡</dt><dd>{selectedFestivalVisitInfo.congestion}</dd></div></dl></div><img src={festivalVisitPlan.image} alt={`${festivalVisitPlan.title} 대표 이미지`}/></div>
          <nav className="festival-plan-tabs" aria-label="방문 정보 상세">{(['시간표','지도','교통'] as const).map((section,index)=><button type="button" className={festivalPlanSection===section?'active':''} key={section} onClick={()=>inspectFestivalPlanSection(section)}><small>0{index+1}</small>{section}</button>)}</nav>
          <div className="festival-plan-primary">
            {festivalPlanSection==='시간표'&&<section className="festival-timeline-detail"><header><small>추천 관람 흐름</small><b>{selectedFestivalVisitInfo.arrivalTip}</b></header>{selectedFestivalVisitInfo.timeline.map(item=><article className="festival-time-row" key={item.time}><time>{item.time}</time><div><b>{item.title}</b><p>{item.detail}</p></div></article>)}</section>}
            {festivalPlanSection==='지도'&&<section className="festival-map-detail"><header><div><small>카카오 지도 검색 주소</small><b>{selectedFestivalVisitInfo.mapAddress}</b></div><MapPin size={19}/></header><div className="festival-zone-list">{selectedFestivalVisitInfo.zones.map((zone,index)=><article key={zone.name}><i>{index+1}</i><div><b>{zone.name}</b><small>{zone.detail}</small></div></article>)}</div><button type="button" className="festival-kakao-map-link" aria-expanded={festivalMapOpen} onClick={()=>setFestivalMapOpen(open=>!open)}><MapPin size={17}/><span><small>패널 안에서 확인</small><b>{festivalMapOpen?'카카오 지도 닫기':'카카오 지도 위치 보기'}</b></span></button>{festivalMapOpen&&<div className="festival-kakao-map-frame"><iframe src={festivalKakaoMapSearchUrl(selectedFestivalVisitInfo.mapAddress)} title={`${selectedFestivalVisitInfo.mapAddress} 카카오 지도`} loading="lazy"/></div>}</section>}
            {festivalPlanSection==='교통'&&<section className="festival-transport-detail"><article><small>도착 추천</small><b>{selectedFestivalVisitInfo.arrivalTip}</b></article><article><small>대중교통</small><b>{selectedFestivalVisitInfo.transport}</b></article><article><small>주차 안내</small><b>{selectedFestivalVisitInfo.parking}</b></article><article><small>귀가 팁</small><b>{selectedFestivalVisitInfo.returnTip}</b></article></section>}
          </div>
          ...
          <section className="festival-route-strip"><Route size={18}/><div><small>추천 방문 코스</small><b>{selectedFestivalVisitInfo.course.join(' → ')}</b></div></section>
        </section>}
