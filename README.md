...
- **17개 세종 공간을 하나의 월드로 연결**하고 공용 포털 좌표와 도착 지점을 서버 정책으로 통일했습니다.
...
3. 동일한 카메라·걷기·달리기 조작으로 17개 월드를 이동하고 공용 고정 포털을 확인합니다.
4. 국립세종수목원에서 꽃을 수집하고 마이홈 꽃밭에 심습니다.
5. 베어트리파크의 곰 가족 포토존과 곰 체험소를 확인한 뒤 미션 보상이 마이홈에 연결되는 흐름을 봅니다.
...
- 공간 안내에 등록된 17개 월드의 카메라 거리, 캐릭터 크기, 걷기·달리기 속도 통일
...
- 마이홈 코티지 GLB, 실내 출입·7개 좌석 상호작용, 곰 FBX 모델과 진행도 기반 보상 오브젝트 표시
...
- 세종호수공원, 공동캠퍼스, 베어트리파크, 정부청사, 세종예술의전당과 축제·먹거리 부스의 확정 포털은 서버에서 고정되어 편집 요청이 차단됩니다.
