...
export function WorldCameraEditor({mapId,canEdit}:{mapId:MapId;canEdit:boolean}){
  ...
  return <aside className={`world-camera-editor${open?' is-open':''}`} onKeyDown={event=>event.stopPropagation()} onKeyUp={event=>event.stopPropagation()}>
    <button type="button" className="world-camera-editor-toggle" onClick={()=>{setOpen(value=>!value);requestProfile()}} aria-expanded={open}><Camera size={16}/><span><small>수목원 포함 맵별 설정</small><b>카메라·캐릭터 조절</b></span><ChevronDown size={15}/></button>
