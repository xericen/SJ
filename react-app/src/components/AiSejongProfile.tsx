...
export function AiSejongProfile({ profile, onClose, onEdit }: { profile: UserProfile; onClose: () => void; onEdit: () => void }) {
  ...
  const topGardenPlants=useMemo(()=>progress.greenhouse.collected.length>=14
  ...
  return <div className="ai-sejong-profile-overlay" role="dialog" aria-modal="true" aria-labelledby="ai-sejong-profile-title">
    <section className="ai-sejong-profile">
      ...
      <section className="profile-card activity-card"><div className="section-title"><h3><BookOpen /> 최근 활동 기록</h3><button onClick={() => setDetail('records')}>전체 기록 보기 <ChevronRight /></button></div><div className="activity-list">{visibleRecords.length ? visibleRecords.map(item => <button type="button" className="activity-row" key={item.id} onClick={()=>setSelectedRecordId(item.id)}><img src={profileAssetUrl(item.image)} alt="" onError={handleProfileImageError}/><div><small>{item.zone}</small><b>{item.title}</b><p>{item.note}</p></div><aside><time>{formatTime(item.at)}</time><strong>+{item.point}P</strong></aside></button>) : <div className="profile-empty"><BookOpen /><b>아직 활동 기록이 없어요</b><p>공간 안에서 관찰·선택·저장·신청 같은 체험을 해보세요.</p></div>}</div></section></div>
...
function DetailPanel({ kind, progress, keywords, savedGroups, savedInterests, experienceProfiles, topGardenPlants, aiSummary, onClose }: {
  ...
  return <div className="profile-detail-backdrop" onClick={onClose}><section className="profile-detail-panel" onClick={event => event.stopPropagation()}><header><div><small>MY SEJONG JOURNEY</small><h2>{titles[kind]}</h2></div><button onClick={onClose}><X /></button></header>
    ...
    {kind === 'records' && <div className="record-detail-list">{progress.records.length ? progress.records.map(item => <article key={item.id}><img src={profileAssetUrl(item.image)} alt="" onError={handleProfileImageError}/><div><small>{item.zone} · {formatTime(item.at)}</small><b>{item.title}</b><p>{item.note}</p><PointBreakdown item={item}/></div><strong>+{item.point}P</strong></article>) : <div className="profile-empty"><BookOpen /><b>첫 활동을 기다리고 있어요</b><p>공간 안에서 선택하거나 체험을 완료하면 구체적인 결과가 기록됩니다.</p></div>}</div>}
