...
import {isWorldCameraEditorMap,resetSharedWorldCameraProfile,saveSharedWorldCameraProfile,WORLD_CAMERA_PROFILE_LIMITS,type WorldCameraProfile} from '../services/worldCameraProfiles';
...
export function WorldCameraEditor({mapId,canEdit}:{mapId:MapId;canEdit:boolean}){
  ...
  const requestProfile=useCallback(()=>gameEvents.emit('world-camera-profile-request',mapId,(value?:WorldCameraProfile)=>{if(value?.mapId===mapId)setProfile(value)}),[mapId]);
  useEffect(()=>{setOpen(false);setMessage('');setProfile(undefined);requestProfile();const timer=window.setTimeout(requestProfile,450);return()=>window.clearTimeout(timer)},[mapId,requestProfile]);
  ...
  const update=(field:Exclude<keyof WorldCameraProfile,'mapId'>,value:number)=>{
    if(!profile)return;
    const next={...profile,[field]:value};setProfile(next);setMessage('미저장 변경');gameEvents.emit('world-camera-profile-preview',next);
  };
  const save=async()=>{if(!profile||saving)return;setSaving(true);try{const result=await saveSharedWorldCameraProfile(profile);setProfile(result.profile);setMessage('공용 저장 완료');gameEvents.emit('world-camera-profile-preview',result.profile)}catch(error){setMessage(error instanceof Error?error.message:'저장하지 못했어요.')}finally{setSaving(false)}};
  const reset=async()=>{if(saving)return;setSaving(true);try{const result=await resetSharedWorldCameraProfile(mapId);gameEvents.emit('world-camera-profile-reset',mapId);setMessage(result.message);window.setTimeout(requestProfile,0)}catch(error){setMessage(error instanceof Error?error.message:'초기화하지 못했어요.')}finally{setSaving(false)}};
