...
export function GamePage({profile,returnState,onExit,onEditProfile,onOpenCommunity}:{profile:UserProfile;returnState?:GameReturnState;onExit:()=>void;onEditProfile:(state:GameReturnState)=>void;onOpenCommunity:(state:GameReturnState)=>void}){
 ...
 const tutorialOpen=lakeTutorialOpen||guideConversation||bearTutorialOpen;
 ...
 {nearbyPortal?.destination==='government'&&buildAiSejongProfile(profile).completion<50&&!mapOverview&&!tutorialOpen&&<section className="portal-charge-panel is-locked"><span>🔒</span><div><small>정부청사 방문 조건</small><b>프로필을 50% 이상 채운 뒤 방문해 주세요</b><em>내 프로필에서 관심사와 체험 기록을 완성하면 해금돼요</em></div></section>}
 {nearbyPortal?.chargeSeconds&&!(nearbyPortal.destination==='government'&&buildAiSejongProfile(profile).completion<50)&&!mapOverview&&!tutorialOpen&&<section className={`portal-charge-panel ${nearbyPortal.theme==='blue'?'is-blue':''} ${guideNearby||mapSignNearby?'with-nearby-actions':''}`}><span>✨</span><div><small>포탈 이동 준비</small><b>{normalizePlaceName(nearbyPortal.label)}(으)로 이동 중</b><div className="portal-charge-steps" style={{gridTemplateColumns:`repeat(${nearbyPortal.chargeSeconds}, minmax(0, 1fr))`}}>{Array.from({length:nearbyPortal.chargeSeconds},(_,index)=><div key={index}><span>{index+1}</span><i><b style={{width:`${Math.max(0,Math.min(1,portalProgress*nearbyPortal.chargeSeconds!-index))*100}%`}}/></i></div>)}</div><em>포탈 안에서 {nearbyPortal.chargeSeconds}초 동안 머물러 주세요</em></div></section>}
 ...
 {<GovernmentAiRecommendationCenter profile={profile} active={location==='중앙광장'} onOpenChange={setGovernmentAiCenterOpen} onNotice={setNotice}/>}
