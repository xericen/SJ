...
export const FIXED_LAKE_RESPAWN:Readonly<RespawnPosition>={x:1105,z:711,yaw:0};
export interface PlayerResumeState extends RespawnPosition { mapId:MapId;savedAt?:number }
export interface PortalPosition { mapId:MapId; destination:MapId; x:number; z:number }
export interface PortalSaveResult { ok:boolean; position?:PortalPosition; message:string }
...
export interface FriendRequest { requestId:string; from:Pick<PlayerState,'id'|'nickname'|'appearance'>; toId:string }
...
export interface ServerToClientEvents {
 worldClock:(serverNow:number)=>void; respawnPositionUpdated:(position:RespawnPosition)=>void; portalEditorPermission:(allowed:boolean)=>void; portalPositionsUpdated:(positions:PortalPosition[])=>void; bearTreePortalPositionsUpdated:(positions:BearTreePortalPositions)=>void; interactionPositionsUpdated:(positions:WorldInteractionPosition[])=>void; lakeExperiencePositionsUpdated:(positions:LakeExperiencePosition[])=>void; campusFeaturePortalPositionsUpdated:(positions:CampusFeaturePortalPosition[])=>void; lakeWishesUpdated:(wishes:LakeWish[])=>void; lakeWishAdded:(wish:LakeWish)=>void; lakeDailyStatsUpdated:(stats:LakeDailyStats)=>void; currentMapUsers:(players:PlayerState[])=>void; userJoined:(player:PlayerState)=>void; userMoved:(player:PlayerState)=>void; userLeft:(id:string)=>void; onlineUsersUpdated:(players:PlayerState[])=>void;
 nearbyChat:(message:ChatMessage)=>void; nearbyChatHistory:(messages:ChatMessage[])=>void; directChatRequested:(request:DirectRequest)=>void; directChatRejected:(data:{requestId:string;byId:string})=>void; directChatStarted:(room:DirectRoom)=>void; directChatHistory:(data:{room:DirectRoom;messages:DirectMessage[]})=>void; directMessageReceived:(message:DirectMessage)=>void; directChatFocusEnded:(data:{directRoomId:string;byId:string})=>void; directChatClosed:(data:{directRoomId:string;byId:string})=>void;
 directChatAvailable:(room:DirectRoom)=>void; directChatResumeRequired:(data:{toId:string})=>void;
 friendRequestReceived:(request:FriendRequest)=>void; friendRequestResolved:(data:{requestId:string;status:'accepted'|'rejected'})=>void; friendState:(data:{friendIds:string[]})=>void;
...
export interface ClientToServerEvents {
 ...
 getPortalPositions:(ack:(positions:PortalPosition[])=>void)=>void; getRespawnPosition:(ack:(position:RespawnPosition)=>void)=>void; saveCurrentPositionAsRespawn:(ack:(result:{ok:boolean;position?:RespawnPosition;message:string})=>void)=>void; getPlayerResumeState:(ack:(position:PlayerResumeState|null)=>void)=>void; migrateBearTreePortalPositions:(positions:BearTreePortalPositions,ack:(result:{ok:boolean;positions:BearTreePortalPositions})=>void)=>void; joinMap:(payload:JoinMapPayload)=>void; changeMap:(payload:JoinMapPayload)=>void; updateMatchProfile:(profile:PublicMatchProfile)=>void; userMoved:(payload:MovementPayload)=>void; savePortalPosition:(position:PortalPosition,ack:(result:PortalSaveResult)=>void)=>void; saveCampusFeaturePortalPosition:(position:CampusFeaturePortalPosition)=>void; saveInteractionPosition:(position:WorldInteractionPosition)=>void; saveLakeExperiencePosition:(position:LakeExperiencePosition)=>void; enterLakeExperience:(experience:LakeExperienceId)=>void; addLakeWish:(message:string,ack:(result:{ok:boolean;wish?:LakeWish;message?:string})=>void)=>void; sendNearbyChat:(message:string)=>void;
 directChatRequest:(toId:string)=>void; directChatAccept:(requestId:string)=>void; directChatReject:(requestId:string)=>void; directMessage:(data:{directRoomId:string;message:string})=>void; directChatFocusEnded:(directRoomId:string)=>void; directChatClosed:(directRoomId:string)=>void;
 directChatResume:(toId:string)=>void;
 friendRequest:(toId:string)=>void; friendAccept:(requestId:string)=>void; friendReject:(requestId:string)=>void; friendRemove:(toId:string)=>void;
