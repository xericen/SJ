...
export function PersonalFarmProgressExperience({mapId,userKey,authenticated,onNotice}:{mapId:MapId;userKey:string;authenticated:boolean;onNotice?:(message:string)=>void}){
  ...
  useEffect(()=>{
    ...
    gameEvents.on('greenhouse-nearby-changed',garden);gameEvents.on('bear-feed-spot-proximity-changed',spot);gameEvents.on('bear-feeding-proximity-changed',bear);gameEvents.on('personal-farm-plant-anchor-proximity-changed',anchor);gameEvents.on('personal-farm-door-proximity-changed',door);gameEvents.on('personal-farm-flower-proximity-changed',planted);
    return()=>{gameEvents.off('greenhouse-nearby-changed',garden);gameEvents.off('bear-feed-spot-proximity-changed',spot);gameEvents.off('bear-feeding-proximity-changed',bear);gameEvents.off('personal-farm-plant-anchor-proximity-changed',anchor);gameEvents.off('personal-farm-door-proximity-changed',door);gameEvents.off('personal-farm-flower-proximity-changed',planted)};
  },[]);
  ...
  },[authenticated,canCollectFlower,canFeedBear,canPickUpFeed,farmAnchor,farmDoor,farmFlower,feedSpot,flowerBedFull,gardenFlower,mapId,pending,pickup,selectedFlower,spotDone]);
  ...
  return <div className="personal-farm-mission-ui" aria-live="polite">
    ...
    {mapId==='garden'&&gardenFlower&&<section className="personal-farm-action-card"><span>{gardenFlower.endsWith('tree')?'🌳':'🌸'}</span><div><small>{greenhousePlantById.get(gardenNearby!)?.displayName??plantName(gardenFlower)}</small><b>{canCollectFlower?'마이홈 수집 기록에 담기':'획득 완료'}</b></div><button type="button" disabled={!canCollectFlower||!!pending} onClick={()=>void run(`flower:${gardenFlower}`,()=>collectGardenFlower(gardenFlower),`${plantName(gardenFlower)}을(를) 수집했어요.`)}>{pending===`flower:${gardenFlower}`?'저장 중…':canCollectFlower?'획득':'완료'}</button></section>}
