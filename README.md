...
이 저장소는 전체 기능을 포함한 원본 프로젝트입니다. WIZ 배포 환경에서는 웹 3D 체험과 WIZ API를 제공하며, Render 서버가 연결된 환경에서는 Socket.IO 기반 실시간 멀티플레이 기능을 제공합니다. 외부 API 키가 없는 로컬 환경에서는 mock provider로 핵심 흐름을 확인할 수 있습니다.
...
| 제공 형태 | 동작 가능한 웹 서비스 + 전체 소스 + 운영·검증 문서 |
...
가입 없이 체험 모드로 전체 공간과 기본 인터랙션을 빠르게 확인할 수 있고, 카카오 로그인에서는 사용자별 누적 기록·협업 프로젝트·개인화 추천이 지속되는 전체 흐름을 확인할 수 있습니다.
...
| 화면 완성도 | 데스크톱에서 안정적인 3D 캔버스를 유지하고 작은 화면은 반응형 레이아웃으로 전환 |
...
- **OpenAI·Kakao Local·공공데이터**를 연결하되, 키가 없는 개발 환경에서는 mock provider로 핵심 흐름을 확인할 수 있습니다.
- **DB를 사용하지 않는 체험 모드**를 제공해 별도 가입 없이 주요 화면과 3D 월드를 바로 확인할 수 있습니다.
- **고정형 데스크톱 캔버스**와 모바일·태블릿 반응형 화면을 함께 제공해 화면 비율 변화에 따른 UI 왜곡을 줄였습니다.
...
| 권장 용도 | 빠른 기능 확인, 가입 없는 데모 | 실제 사용자 활동 누적, 장기 프로필 성장, 협업 프로젝트와 개인화 일정 저장 |
...
```text
사용자 입력·3D 상호작용
  ├─ 이동·체류·콘텐츠 선택 → Experience Harness → 행동 이벤트 정규화
  ├─ 꽃·먹이·보상          → Personal Farm API → 서버 규칙 검증
  ├─ 모집·동아리·프로젝트   → WIZ App API + MySQL → 계정별/공용 상태 분리
  └─ 캐릭터·채팅·협업       → Socket.IO Room → 참가자에게 변경 전파
                                      ↓
                         프로필 집계·OpenAI 분석
                                      ↓
                       Kakao Local 후보 검증·코스 생성
```

- 카카오 사용자는 MySQL 스냅샷을 원본으로 사용하며 로그인 시 브라우저 활동 캐시를 서버 데이터로 교체합니다. 체험용 사용자는 별도 저장 키를 사용해 계정 데이터와 섞이지 않습니다.
- 모집글(`recruitment`)과 프로젝트글(`project`)은 종류를 명시해 모집센터와 프로젝트실에서 서로 다른 목록으로 조회합니다.
- 프로젝트 협업 초안은 revision을 증가시키며 역할·아이디어·테마·먹거리·코스를 공유합니다. 역할 전용 상태와 초안 역할을 함께 갱신하고, 조회 요청은 캐시를 사용하지 않습니다.
- 운영 iframe과 React 번들은 동일한 `RUNTIME_BUILD_ID`를 사용합니다. 빌드 시 두 값이 다르거나 엔트리 자산이 누락되면 회귀 검사가 실패합니다.

### AI 추천 안전장치

| 단계 | 안전장치 |
|---|---|
| 입력 구성 | 사용자가 입력하거나 서비스에 저장된 프로필·행동·프로젝트 데이터만 사용 |
| 장소 후보 | Kakao Local에서 조회된 세종 후보 장소를 모델 입력으로 제한 |
| 모델 출력 | 구조화된 JSON 형태와 최대 장소 수를 검증하고 후보 밖의 장소는 제외 |
| 장애 처리 | 외부 API 제한시간과 명시적인 오류 상태를 적용하며 운영에서는 임의 더미 코스로 성공 처리하지 않음 |
| 개인정보 | 브라우저 번들에 비밀키를 포함하지 않고 서버 세션과 사용자 ID 기준으로 데이터를 격리 |

### 상태 일관성과 동시 편집

- 활동 상태는 로그인 계정, 체험 세션, 공용 월드 설정을 서로 다른 저장 영역으로 분리합니다.
- 프로젝트 자동 저장은 순차 큐로 처리해 이전 요청이 최신 내용을 덮어쓰지 않도록 하고, 새로고침은 대기 중인 저장이 끝난 뒤 서버 revision을 조회합니다.
- 아이디어와 대화는 ID 기준으로 누적 병합하고, 역할 변경은 팀장 권한을 서버에서 다시 검사합니다.
- 프로젝트 완성은 참가자별 동의 상태를 저장하고 전원 동의가 확인된 경우에만 완료 상태로 전환합니다.
- 개인 팜은 꽃 해금·심기, 먹이 수집·급여 조건을 서버에서 재검증해 클라이언트 상태 조작을 차단합니다.
