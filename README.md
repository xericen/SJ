...
정부청사 → 중앙광장 AI 추천센터 · 전망대 · 스마트시티
                                      ↓
실시간 AI 세종 프로필 + 맞춤 코스 + 조치원 생활권 탐험
```
...
현재 런타임은 22개 `MapId`를 사용하며, 아래처럼 연결됩니다.
...
| 정부청사 | 정부청사, 중앙광장, 정책 체험관, 전망대, 세종 스마트시티 국가시범도시 | AI 추천센터, 정책·전망·스마트시티 체험 | 전망대·스마트시티 방문 기록과 전체 행동 기반 종합 분석 |
| 조치원 생활권 | 조치원역, 세종전통시장, 조치원공원, 대학로 | 역·시장·공원·청년 문화 거리 탐험 | 장소 방문과 지역 관심 기록 |
...
세종한바퀴는 세종시의 생활권, 공공 공간, 문화·관광 자원과 지역 커뮤니티를 하나의 3D 월드로 연결한 체험형 AI 메타버스 플랫폼입니다. 사용자는 자신의 캐릭터로 현재 22개 맵을 탐험하고, 지역 이웃·동아리·모집글·프로젝트를 발견하며, Experience Harness가 기록한 행동을 AI가 해석해 개인화된 세종 프로필과 맞춤 코스로 연결합니다.
...
| 완성도 | 22개 맵, 로그인·체험 모드, 생태 미션, 커뮤니티, AI 추천을 실제 운영 URL에서 제공 |
...
- **22개 세종 맵을 하나의 월드로 연결**하고 공통 이동 조작, 지형별 카메라 프로필, 공용·로컬 포털 정책과 안전 도착 지점을 적용했습니다.
...
3. 공통 걷기·달리기 조작과 공간별 지형에 맞춘 카메라로 22개 맵을 탐색합니다. 권한 계정에서는 지원 맵의 카메라 프로필을 조정해 모든 사용자에게 공유할 수 있습니다.
...
| 구분 | 체험용 | 카카오 로그인 |
|---|---|---|
| 프로필 저장 위치 | 브라우저의 `localStorage`·`sessionStorage` | WIZ 계정과 MySQL 기반 서버 저장 |
| 프로필 범위 | 닉네임·관심사·캐릭터를 임시로 작성해 바로 체험 | 닉네임·관심사·생활권·캐릭터와 행동·미션 진행도를 계정에 연결 |
| 행동 기록 | 현재 브라우저에서만 유지되는 오프라인 체험 데이터 | Experience Harness, 개인 팜 진행도, 커뮤니티 활동을 사용자별로 저장 |
| 유지 기간 | 브라우저 세션 종료·체험 초기화 시 사라질 수 있음 | 다시 로그인해도 프로필과 진행 기록 복원 |
| 목적 | 가입 없이 3D 공간과 주요 흐름을 빠르게 확인 | 세종 AI 프로필 성장, 마이홈 보상, 맞춤 코스 추천을 지속 이용 |

체험용은 저장된 프로필을 서버 계정으로 자동 이전하지 않습니다. 계속 기록을 남기고 여러 기기에서 이어서 이용하려면 카카오 로그인 후 프로필을 작성해야 합니다.

### 3D 세종 월드

- 세종 생활권 및 주요 장소를 3D 월드로 탐색
- 현재 22개 맵의 이동 속도를 통일하고 호수공원·베어트리파크·마이홈·공동캠퍼스 등은 지형과 실내 구조에 맞춘 카메라 프로필 적용
