import type {ArtsCenterPerformance} from '../game/artsCenterPerformances';
...
export const renderPerformanceSummaryDocument=(performance:ArtsCenterPerformance,imageUrl:string)=>`<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(performance.title)}</title><style>*{box-sizing:border-box}body{max-width:820px;margin:0 auto;padding:24px;background:#f5f8f7;color:#213b35;font:14px/1.65 system-ui,sans-serif}main{overflow:hidden;border:1px solid #dfe8e5;border-radius:18px;background:#fff;box-shadow:0 10px 28px #173c3220}.hero{padding:10px;background:${escapeHtml(performance.color)}}.hero img{display:block;width:100%;height:auto;border-radius:11px}.content{padding:22px}.content small{color:${escapeHtml(performance.accent)};font-size:11px;font-weight:900}.content h1{margin:5px 0 7px;font-size:22px;line-height:1.35}.content>p{margin:0 0 18px;color:#60766f}.content dl{margin:0;display:grid;grid-template-columns:1fr 1fr;gap:9px}.content dl div{padding:11px;border-radius:10px;background:#f3f7f5}.content dl .wide{grid-column:1/-1}.content dt{color:#6e817b;font-size:10px;font-weight:800}.content dd{margin:3px 0 0;font-size:12px;font-weight:850;overflow-wrap:anywhere}@media(max-width:520px){body{padding:12px}.content{padding:17px}.content dl{grid-template-columns:1fr}.content dl .wide{grid-column:auto}}</style></head><body><main><div class="hero"><img src="${escapeHtml(imageUrl)}" alt="${escapeHtml(performance.title)} 공연 포스터"></div><section class="content"><small>${escapeHtml(performance.category)} · 세종예술의전당 공식 공연</small><h1>${escapeHtml(performance.title)}</h1><p>${escapeHtml(performance.description)}</p><dl><div><dt>공연 일정</dt><dd>${escapeHtml(performance.date)}</dd></div><div><dt>장소</dt><dd>${escapeHtml(performance.venue)}</dd></div><div><dt>관람 등급</dt><dd>${escapeHtml(performance.age)}</dd></div><div><dt>러닝타임</dt><dd>${escapeHtml(performance.runtime)}</dd></div><div class="wide"><dt>가격</dt><dd>${escapeHtml(performance.price)}</dd></div><div><dt>주최</dt><dd>${escapeHtml(performance.host)}</dd></div><div><dt>주관</dt><dd>${escapeHtml(performance.organizer)}</dd></div><div class="wide"><dt>문의</dt><dd>${escapeHtml(performance.inquiry)}</dd></div></dl></section></main></body></html>`;
